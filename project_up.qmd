...
staff_comparison = pd.DataFrame({'National Staff': nationals, 'International Staff': internationals})

# Create horizontal stacked bar chart
fig, ax = plt.subplots(figsize=(8, 5))

# Plot the bars
staff_comparison.plot(kind='barh', ax=ax, color=[main_palette[3], accent_palette[3]])

# Add totals as annotations with contrasting colors for better visibility
for i, row in enumerate(staff_comparison.index):
    ...
    # National staff numbers (inside blue bars)
    ax.text(nat_val / 2, i, f"{int(nat_val)}", 
            ha='center', va='center', color='white', fontweight='bold',
            path_effects=[pe.withStroke(linewidth=3, foreground='black')])
    
    # International staff numbers (inside orange bars)
    ax.text(nat_val + int_val / 2, i, f"{int(int_val)}", 
            ha='center', va='center', color='white', fontweight='bold',
            path_effects=[pe.withStroke(linewidth=3, foreground='black')])
    
    # Ratio text (outside bars)
    ax.text(nat_val + int_val + 10, i, f"({ratio:.1%} national)", 
